...
export const paymentsApi = {
  getVnPayUrl: async (data: VnPayUrlRequest): Promise<VnPayUrlResponse> => {
    const response = await apiClient.post<VnPayUrlResponse>('/api/payments/vnpay/create', data);
  ...
  processReturn: async (queryParams: URLSearchParams): Promise<VnPayReturnResponse> => {
    ...
    const response = await apiClient.get<VnPayReturnResponse>(`/api/payments/vnpay/return?${queryString}`);
    return response.data;
  },
};
